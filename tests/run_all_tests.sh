...
echo "[1/8] Running test_tong_hop_link.mjs..."
node tests/test_tong_hop_link.mjs

echo "[2/8] Running test_telegram_phanserver.mjs..."
node tests/test_telegram_phanserver.mjs

echo "[3/8] Running test_fleet_state_2pc.mjs..."
node tests/test_fleet_state_2pc.mjs

echo "[4/8] Running test_pairing.mjs..."
node tests/test_pairing.mjs

echo "[5/8] Running delta updater tests..."
python3 -m unittest discover -s delta/tests

echo "[6/8] Running device agent tests..."
python3 -m unittest discover -s agent/tests

echo "[7/8] Running onboarding installer tests..."
python3 tests/test_onboarding_install.py

echo "[8/8] Running E2E flow tests..."
